...
// offset `offset`.
#define REG32_PTR(instance, offset) ((guint32*) (instance + offset))
...
{
    if (instance_number < 1 || instance_number > 12) {
        errno = EDOM;
        return MAP_FAILED;
    }
    int instance_addr = gpt_instance_addrs[instance_number - 1];
...
pwm_config_timer(guint8 *instance, guint32 resolution, float duty_cycle)
{
    ...
    guint32 dc = 0xffffffff - ((guint32) (resolution * (1.0 - duty_cycle)));
